...
Author: Rizwan Aarif
This program finds the total ratings of all the movies. */
...
object MostRated100k {  
  ...
  def main(argv: Array[String]) {     
    ...
    val conf = new SparkConf().setAppName("MostRated100k")
    ...
    val lines = sc.textFile(args.input() + "/u.data")
  ...
  def MovieNames(path: String) : Map[Int, String] = {
    ...
    var MoviesCollection:Map[Int, String] = Map()
     ...
     val lines = Source.fromFile(path + "/u.item").getLines()
     for (line <- lines) {

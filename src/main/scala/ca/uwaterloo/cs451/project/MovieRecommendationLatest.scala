...
object MovieRecommendationLatest {
	val log = Logger.getLogger(getClass().getName())
  
  def main(args: Array[String]) {
    
    val conf = new SparkConf().setAppName("MovieRecommendationLatest")
    val sc = new SparkContext(conf)
    
    val textfile = sc.textFile("ml-latest-small/ratings.csv")
    ...
    if (args.length > 0) {
      ...
      println("\nTop 10 similar movies for " + moviesWithNames(movieID))
      for (result <- results) {
        val sim = result._1
        val pair = result._2
